<?php
    $lst_customers=DP::run_query('SELECT `id`, `name`, `des`, `rank`, `logo`, `link`, `status` FROM `customer` order by `id` desc',[],2);
?>
        ...
        <div class="row">
            <form action="" enctype="multipart/form-data" method="POST">
                ...
                <div class="form-group col-md-3">
                    ...
                    <input required type="text" class="form-control" id="link" name="link" value="http://">
                </div>
                <div class="form-group col-md-3">
                ...
                <div class="col-md-12">
                    <button name="btn_add" class="btn"
                        style="color:#fff;background-color: #152036;border:solid 1px #fff;"><i
                            class="fa fa-save"></i>&nbsp;<span>Thêm khách hàng</span></button>
                </div>
            </form>
        </div>
    </div>
...
$('[name=btnEdit]').click(function(){   
    $.post("api/get_list_cus.php", {
        ...
        function(data, status) {
            ...
            $('input[name="des"]').val(r[0].des);
            $('input[name="rank"]').val(r[0].rank);
            $('#img_ico').attr('src','../img/logocus/'+r[0].logo);
            $('input[name="link"]').val(r[0].link);
            $('input[name="ddl_tt"]').val(r[0].status);
            $('input[name="hdf_id"]').val(r[0].id);
            $('input[name="hdf_img"]').val(r[0].logo);
        ...
        function(data, status) {
            ...
            for(var i=0;i<r.length-2;i++)
            {
                ...
                '<tr>'+
                    ...
                    '<td>'+r[i].name+'</td>'+
                    '<td>'+r[i].des+'</td>'+
                    '<td><img src="../img/logocus/'+r[i].logo+'"/></td>'+
                    '<td>'+r[i].link+'</td>'+
                    '<td><button cus="'+r[i].id+'" name="btn_stt" class="'+ (r[i].status=='1'?'pd-setting':'ds-setting')+'">'+(r[i].status=='1'?'Hoạt động':'Ẩn')+'</button></td>'+
                    '<td><button data-toggle="tooltip" title="" class="pd-setting-ed" data-original-title="Edit" type="button" data-id="'+r[i].id+'" name="btnEdit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>';
                if(r[i].status=='1')
                {
                    sHTML=sHTML+
