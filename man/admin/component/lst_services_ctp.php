...
$query ='SELECT `id`, `tit`,`des`, `content`, `create_time`, `create_mem`, `edit_time`, `edit_mem`, `status`, `parent_id`, `rank` FROM `services`';
$paras=[];
...
<div class="product-status-wrap">   
    <div>
        <button class="btn" style="color:#fff;background-color: RGBA(0,0,0,0.1);border:solid 1px #fff" onclick="window.location.href='service.php'"><i class="fa fa-edit"></i>Đăng bài</button>
    </div>                   
    ...
    <table id="tb-post">
        <tbody>
            <tr>
                ...
                <th style="min-width:20%;">Tên dịch vụ</th>
                <th>Mô tả ngắn</th>
                <th>Dịch vụ cha</th>
                <th>Trạng thái</th>
            ...
            <tr>
                ...
                <td><a href="service.php?id=<?php echo $post['id'];?>"><?php echo $post['tit']?></a></td>
                ...
                <?php
					...
					{
                        if($post['parent_id']==$ser['id']) $parent= $ser['tit'];
                ...
                <td>
                    <button data-toggle="tooltip" title="" class="pd-setting-ed" data-original-title="Edit" onclick="window.location.href='service.php?id=<?php echo $post['id'];?>'"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>
                </td>
            </tr>
            <?php
                }
